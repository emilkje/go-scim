...
func NewUserResourceHandler() *testResourceHandler {
...
func (h testResourceHandler) GetAll(r *http.Request, params scim.ListRequestParams) (scim.Page, error) {
	...
	if params.FilterValidator != nil {
		fmt.Println("filter validator present")
		exp := params.FilterValidator.GetFilter()
		fmt.Printf("filter expression: %v\n", exp)
	}
	...
	for k, v := range h.data {
		...
		if i >= params.StartIndex {
			...
			if err := params.FilterValidator.Validate(); err != nil {
				fmt.Printf("filter not valid: %s", err.Error())
				continue
			}

			if err := params.FilterValidator.PassesFilter(v.resourceAttributes); err != nil {
